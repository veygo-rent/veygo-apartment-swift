...
struct PhoneVerifyView: View {
    ...
    var body: some View {
        VStack(spacing: 24) {
            ...
            HStack(spacing: 12) {
                InputWithInlinePrompt(userInput: .constant(session.renter?.phone ?? "Not set"), promptText: "Your Phone number")
                    .disabled(true)
    ...
    private func sendCodeRequest() async throws {
        ...
        let body = ["verification_method": "Phone"]
    ...
    private func verify() async {
        ...
        do {
            ...
            session.setRenter(renter)   // refresh profile with verified phone
            dismiss()
        } catch let error as VeygoError {
            switch error {
            case .unauthorized:
